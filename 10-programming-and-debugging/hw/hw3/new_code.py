...
def checkver(pth):
    chk = subprocess.check_output([sys.executable, pth], encoding='UTF-8')
...
def check4():
    ...
    chk = subprocess.check_output([p, 'ldd.py'], encoding='UTF-8')
...
def check5():
    try:
        ...
        socket.socket(socket.AF_INET, socket.SOCK_STREAM).connect(('ya.ru', 80))
